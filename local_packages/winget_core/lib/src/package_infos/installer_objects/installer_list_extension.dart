...
extension InstallerList on Iterable<Installer> {
  ...
  String uniquePropertyNames(PackageLocalizer localizer) {
    Map<PackageAttribute, bool> uniqueProperties =
        areIdentifyingPropertiesUnique()
          ..removeWhere((key, value) => value == false);
    List<String> names =
        uniqueProperties.keys.map((e) => e.title(localizer)).toList();
...
class MultiProperty {
  ...
  String title(PackageLocalizer localizations, LocalizationName localeNames) {
